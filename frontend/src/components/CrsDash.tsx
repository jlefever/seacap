...
import { createDeps, createEntities, isM2m } from "../util";
    ...
    override render() {
        ...
        return <>
            ...
            <div className="content">
                The file <strong><GithubLink path={sum.center} repo={repo} /></strong> is depended on by <strong>{sum.fanout}</strong> files and has co-changed at least twice with <strong>{sum.evoFanout}</strong> of them. Additionally, this file depends on <strong>{sum.fanin}</strong> files and has co-changed at least twice with <strong>{sum.evoFanin}</strong> of them. So this is a crossing with a total of <strong>{sum.size}</strong> files.
            </div>
            ...
            <div className="columns">
                <div className="column">
                    <h2 className="title is-4">Alignment</h2>
                    There are <strong>X</strong> method-to-method dependencies which make up the fan-in portion of this crossing.
                    <ul>{evoInDeps.filter(d => d.target.kind == "method").map(d => <EntityDiv entity={d.target} repo={repo} />)}</ul>
                    There are <strong>Y</strong> method-to-method dependencies which make up the fan-out portion of this crossing.
                    <ul>{evoOutDeps.filter(d => d.source.kind == "method").map(d => <EntityDiv entity={d.source} repo={repo} />)}</ul>
                    {/* The file <strong><GithubLink path={sum.center} repo={repo} /></strong> has <strong>X</strong> entities (<strong>Y</strong> of which are methods). */}
                    {/* {Array.from(entities.values()).map(e => <div key={e.id}><EntityDiv entity={e} repo={repo} /></div>)} */}
                </div>
            </div>
        </>;
    }
